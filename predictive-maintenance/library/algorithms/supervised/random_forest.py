"""
Random Forest algorithm adapter.
"""
...
class RandomForestAdapter(BaseAlgorithm):
    """Adapter for scikit-learn Random Forest algorithm"""
    ...
    def train(self, X: pd.DataFrame, y: Optional[pd.Series] = None) -> TrainingMetrics:
        ...
        if y is None:
            raise ValueError("Random Forest requires target variable y")
        ...
        if self.config.task_type == TaskType.REGRESSION:
            self.model = RandomForestRegressor(
                random_state=self.config.random_state, **self.config.hyperparameters
            )
        else:
    ...
    def predict(self, X: pd.DataFrame) -> PredictionOutput:
        ...
        return PredictionOutput(
            ...
            metadata={"algorithm": "random_forest", "n_samples": len(X)},
        )

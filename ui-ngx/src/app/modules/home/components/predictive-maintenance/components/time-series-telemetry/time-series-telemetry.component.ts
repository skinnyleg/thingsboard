...
import { LogEntry } from '@home/components/predictive-maintenance/components/anomalies/anomalies.component';
...
export class TimeSeriesTelemetryComponent implements OnInit, OnDestroy, AfterViewInit, OnChanges {
  ...
  @Input() title: string = 'Sensor Telemetry';
  @Input() forecastId: string; // Forecast model ID for fetching predictions
  @Input() showForecast: boolean = false; // Enable/disable forecast display
  ...
  // Forecast data
  private forecastData: Array<[number, number]> = [];
  private forecastMaxSteps: number = 0;
  ...
  ngAfterViewInit(): void {
    ...
    // Subscribe to forecast if forecast ID is available
    if (this.forecastId && this.showForecast) {
  ...
  private subscribeToForecast(): void {
    if (!this.forecastId || !this.showForecast) {
      console.log('Skipping forecast subscription - forecastId:', this.forecastId, 'showForecast:', this.showForecast);
      return;
    }

    console.log(`Subscribing to forecast for forecastId: ${this.forecastId}, sensor: ${this.selectedSensor}`);

    // Subscribe to forecast WebSocket logs
    const logsObservable = this.modelWebSocketService.requestJobLogs(this.forecastId)
      .pipe(mergeMap((msg) => flatMap(msg.data.logs)));

    // Filter for forecast prediction logs
    const forecastPredictionLogs$ = logsObservable.pipe(
      filter((log: LogEntry) =>
    ...
    // Subscribe to forecast predictions
    this.forecastSubscription = forecastPredictionLogs$.subscribe({
      next: (log: LogEntry) => {
        this.handleForecastUpdate(log);
      },
      error: (error) => {
        console.error('Error receiving forecast data:', error);
      }
    });
  }

  private handleForecastUpdate(log: LogEntry): void {
    try {
      console.log('Forecast log received:', log);
      ...
      if (typeof log.message === 'string') {
        ...
        results = log.message.result || log.message;
      }

      // Extract forecast_max_steps
      if (results.forecast_max_steps !== undefined) {
        this.forecastMaxSteps = results.forecast_max_steps;
        delete results.forecast_max_steps;
      }

      // Get forecast data for the selected sensor
      if (results[this.selectedSensor]) {
        const sensorForecast = results[this.selectedSensor];

        // Convert forecast data to [timestamp, value] format
        if (sensorForecast.forecast && sensorForecast.timestamp) {
          this.forecastData = sensorForecast.timestamp.map((ts: number, index: number) => {
            return [ts, sensorForecast.forecast[index]];
          });

          console.log(`Loaded ${this.forecastData.length} forecast points for sensor ${this.selectedSensor}`);
        ...
        console.warn(`No forecast data found for sensor: ${this.selectedSensor}`);
      }
    } catch (error) {
  ...
  private updateChart(): void {
    ...
    if (this.showForecast && this.forecastData && this.forecastData.length > 0) {
      seriesArray.push({
        name: `${this.selectedSensor} Forecast`,
        type: 'line',
    ...
    const legendData = this.showForecast && this.forecastData && this.forecastData.length > 0
      ? [this.selectedSensor, `${this.selectedSensor} Forecast`]
      : [this.selectedSensor];
      ...
      this.attributeService.getEntityTimeseries(
        ...
        next: (data) => {
          ...
          if (data && data[this.selectedSensor]) {
            ...
            let allDataPoints: Array<[number, number]> = historicalPoints.map((point: any) => {
              return [point.ts, parseFloat(point.value)];
            });

            // Filter out data points outside the actual requested time range
            // Keep data points within [minTime, maxTime]
            this.telemetryData = allDataPoints.filter(point => {
              return point[0] >= minTime && point[0] <= maxTime;
            });
  ...
  onSensorChange(): void {
    ...
    // Unsubscribe from current forecast
    if (this.forecastSubscription) {
      this.forecastSubscription.unsubscribe();
    }
    ...
    // Resubscribe to forecast for the new sensor
    if (this.forecastId && this.showForecast) {
      this.subscribeToForecast();
    }
  }

...
export class AttributeDatasource implements DataSource<AttributeData> {
  ...
  private pageDataSubject = new BehaviorSubject<PageData<AttributeData>>(
    emptyPageData<AttributeData>()
  );
  ...
  ): Observable<AttributeData[] | ReadonlyArray<AttributeData>> {
  ...
  loadAttributes(
    entityId: EntityId,
    attributesScope: TelemetryType,
    pageLink: PageLink,
    reload: boolean = false
  ): Observable<PageData<AttributeData>> {
    if (reload) {
    ...
    this.fetchAttributes(entityId, attributesScope, pageLink)
      .pipe(catchError(() => of(emptyPageData<AttributeData>())))
      .subscribe((pageData) => {
    ...
    return this.getAllAttributes(entityId, attributesScope).pipe(
      map((data) => {
        const filteredData = data.filter(
          (attrData) => attrData.lastUpdateTs !== 0 && attrData.value !== ""
        );
  ...
  getAllAttributes(
    entityId: EntityId,
    attributesScope: TelemetryType
  ): Observable<Array<AttributeData>> {
    if (!this.allAttributes) {
      ...
      if (isClientSideTelemetryType.get(attributesScope)) {
        ...
        attributesObservable = this.attributeService.getEntityAttributes(
          entityId,
          attributesScope as AttributeScope
        );
      }
  ...
  isEmpty(): Observable<boolean> {
    return this.attributesSubject.pipe(map((attributes) => !attributes.length));
  }

  total(): Observable<number> {
    return this.pageDataSubject.pipe(map((pageData) => pageData.totalElements));
  }
